...
import { mockStudents, mockParents, mockGrades, mockSections } from '../constants/mockData';

export const Students: React.FC<{ academicYear: string }> = ({ academicYear }) => {
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3 w-full lg:w-auto">
              ...
              >
                ...
                {mockGrades.map(g => <option key={g.id} value={g.name}>{g.name}</option>)}
              </select>
              <select 
                value={sectionFilter}
              ...
              >
                ...
                {Array.from(new Set(mockSections.map(s => s.name))).map(s => <option key={s} value={s}>{s}</option>)}
              </select>
                            ...
                            <div className="flex items-center justify-end gap-2">
                              {!student.parentId && (
                                <button 
                                  ...
                                  className="p-2 bg-primary/5 text-primary rounded-lg hover:bg-primary hover:text-white transition-all"
                                  title="Link Parent"
                                >
                                  <Link2 className="w-4 h-4" />
                                </button>
              ...
              <div className="flex-1 overflow-auto p-8 space-y-8">
                <div className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest mt-1">ID: {selectedStudent.id}</p>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-slate-100 space-y-4">
                  ...
                  <div className="space-y-2">
                    <button 
                      onClick={() => {
                        const parent = mockParents.find(p => p.id === selectedStudent.parentId);
                ...
                {selectedStudent.parentId && (
                  <div className="pt-6 border-t border-slate-100 space-y-4">
                    ...
                    {(() => {
                      const parent = mockParents.find(p => p.id === selectedStudent.parentId);
                      if (!parent) return <p className="text-xs text-slate-400 font-bold">Parent data not found</p>;
                      return (
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="grid grid-cols-3 gap-6">
                  <div className="space-y-2">
                    ...
                    <select defaultValue={selectedStudent.grade} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 outline-none focus:ring-2 focus:ring-primary/20 transition-all">
                      {mockGrades.map(g => <option key={g.id}>{g.name}</option>)}
                    </select>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Section</label>
                    <select defaultValue={selectedStudent.section} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 outline-none focus:ring-2 focus:ring-primary/20 transition-all">
                      <option>A</option>
                      <option>B</option>
                      <option>C</option>
                    </select>
                  ...
                  <div className="space-y-2">
                    ...
                    <select defaultValue={selectedStudent.registrationStatus} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 outline-none focus:ring-2 focus:ring-primary/20 transition-all">
              ...
              <div className="p-8 border-t border-slate-100 flex items-center justify-end gap-4">
                ...
                <button 
                  onClick={() => setIsEditModalOpen(false)}
                  className="bg-primary text-white px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-primary/20 hover:shadow-xl active:scale-95 transition-all"
                >
                  Save Changes
                </button>
              </div>
        ...
        {studentToLink && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="mt-8 space-y-4">
                ...
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-900 uppercase">Mamo Kassa</p>
                      <p className="text-[10px] text-slate-500">+251 987 654 321</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => {
                      setStudents(prev => prev.map(s => s.id === studentToLink.id ? { ...s, parentId: 'p2' } : s));
              ...
              <button 
                onClick={() => setStudentToLink(null)}
                className="w-full mt-6 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-all font-sans"
              >
                Maybe Later
              </button>
            </motion.div>
              ...
              <div className="p-8 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input type="text" placeholder="e.g. John Doe" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 outline-none focus:ring-2 focus:ring-primary/20 transition-all" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Student ID</label>
                    <input type="text" placeholder="ST-2025-..." className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 outline-none focus:ring-2 focus:ring-primary/20 transition-all" />
                  </div>
                </div>
