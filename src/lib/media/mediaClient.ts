...
export function createMediaClient(options: MediaClientOptions) {
  ...
  async function initiateUpload(file: File): Promise<MediaUploadInitResponse> {
    const response = await apiRequest<MediaUploadInitResponse | ApiEnvelope<MediaUploadInitResponse>>(
      '/api/media/upload',
      {
        method: 'POST',
        body: JSON.stringify({
  ...
  async function getPartUrl(params: {
    ...
    const response = await apiRequest<MediaPartUrlResponse | ApiEnvelope<MediaPartUrlResponse>>(
  ...
  async function getMedia(mediaId: MediaId): Promise<MediaMetadata> {
    const response = await apiRequest<MediaMetadata | ApiEnvelope<MediaMetadata>>(
      `/api/media/${mediaId}`,
      {
        method: 'GET',
      },
