...
export default function CompleteInvitationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push(
          "/?message=Your account has been activated successfully! Please sign in with your new password."
        )
      }, 2000)
    } catch (err: any) {
      let errMsg =
      ...
      >
        ...
        <div className="flex flex-col items-center rounded-[2rem] border border-border bg-white/70 p-8 shadow-[0_20px_50px_rgba(0,0,0,0.06)] backdrop-blur-xl md:p-10">
          ...
          <div className="mb-8 flex flex-col items-center gap-3 text-center">
            <div className="flex h-14 w-14 shrink-0 animate-bounce items-center justify-center rounded-2xl bg-gradient-to-tr from-primary to-accent shadow-lg shadow-primary/20">
            ...
            {isSuccess ? (
              ...
              >
                ...
                <p className="max-w-xs text-sm font-medium text-muted-foreground">
                  Your new password is set. Redirecting you to the sign in
                  portal...
                </p>
              </motion.div>
            ) : (
              ...
              >
                ...
                <p className="mb-6 text-center text-xs leading-relaxed font-medium text-slate-500">
                  Welcome to the Kelem Branch Administrator team! Please
                  configure your secure password below to activate your
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="space-y-2 rounded-2xl border border-slate-100 bg-slate-50 p-3.5 text-xs font-semibold text-slate-500">
                    <div className="flex items-center gap-2">
                      <div
                        className={`h-1.5 w-1.5 rounded-full ${isMinLength ? "bg-emerald-500" : "bg-slate-300"}`}
                      />
                      <span>At least 8 characters long</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div
                        className={`h-1.5 w-1.5 rounded-full ${isMatching ? "bg-emerald-500" : "bg-slate-300"}`}
                      />
